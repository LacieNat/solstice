...
$wd = dirname(__FILE__);      //this is the only thing that needs to change from project to project
$reposURL = 'git@genie.solstice.sg:Cytey/solstice.git';
  ...
  if( $HTTP_RAW_POST_DATA ){
    if( $oData = json_decode( $HTTP_RAW_POST_DATA ) ){ //transform the string into an object so we can get to the 'ref' element
      ...
      if( ( $branch = array_pop( preg_split("/[\/]+/", $oData->ref) ) ) != "heads" ){
			if(isset($specificBranch) && $branch == $specificBranch) {
				if( is_dir( "$wd" ) ){   //lets check if branch dir exists
					...
					$result = syscall("git pull origin $branch", "$wd");
				} else {
